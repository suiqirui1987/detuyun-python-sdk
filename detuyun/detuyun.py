...
class DetuYunServiceException(Exception):
...
class DetuYunClientException(Exception):
...
class DetuYun:
    ...
    def getlist(self, key='/'):
        ...
        if content == "":
            return []
        items = content.split('\n')
        return [dict(zip(['name', 'type', 'size', 'time','filetype'],
    ...
    def __do_http_request(self, method, key,
        ...
        if isinstance(value, file):
            ...
            raise DetuYunClientException("object type error")
    ...
    def __make_signature(self, method, uri, date, length):
        signstr = '&'.join([ uri, date, self.password])
        signature = hashlib.md5(signstr).hexdigest()
        return 'DetuYun ' + self.username + ':' + signature

    def __make_user_agent(self):
        default = "test/" + __version__
    ...
    def __get_meta_headers(self, headers):
        return dict(iter([(k[10:].lower(), v) for k, v in headers
                    if k[:10].lower() == 'x-detuyun-']))
    ...
    def __do_http_basic(self, method, uri,
        ...
        try:
            ...
            raise DetuYunClientException(str(e))
        ...
        if msg:
            raise DetuYunServiceException(status, msg, err)
        return content
